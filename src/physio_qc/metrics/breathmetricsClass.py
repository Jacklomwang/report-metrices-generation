...
class bmObject:
    """Class object that handles breathing data (spirometry in our lab's case).

    Updated by Andrew Xie (2026) to better align with original MATLAB implementation.
    Originally created in MATLAB and translated in Python by Torben Noto at the Zelano Lab.
    Original MATLAB code available at https://github.com/zelanolab/breathmetrics
    Noto's Python implementation available at https://github.com/torbenator/breathmetrics_python/tree/0807d3fd832a7518a569172f41fed0ab0ca478e7
    ...
    Parameters
    ----------
    resp : Your breathing signal in the form of a list
    ...
    def fftSmooth(self, resp, srateCorrectedSmoothedWindow):
        resp = np.atleast_1d(resp)
        if resp.ndim > 1 and resp.shape[0] < resp.shape[1]:
            resp = resp.T

        # Create the window for smoothing. This is a rectangular window of length srateCorrectedSmoothedWindow, centered on the middle of the data.
        L = len(resp)
        window = np.zeros(L)
        window[int(np.floor((L - srateCorrectedSmoothedWindow + 1) / 2))-1:
            int(np.floor((L + srateCorrectedSmoothedWindow) / 2))] = 1
        
        tmp = np.real(np.fft.ifft(np.fft.fft(resp) * np.fft.fft(window) / srateCorrectedSmoothedWindow))
        y = -1 * np.fft.ifft(np.fft.fft(-1 * tmp) * np.fft.fft(window) / srateCorrectedSmoothedWindow)
    ...
    def smoothResp(self):
        ...
        self.smoothedRespiration = self.fftSmooth(self.resp, srateCorrectedSmoothedWindow)
    ...
    def correctRespirationToBaseline(self, method='simple', zScore=0, verbose=0):
        ...
        thisResp = sp.signal.detrend(self.smoothedRespiration)
        respMean = thisResp - np.mean(thisResp)
        ...
        if method == 'simple':
            ...
            if verbose == 1:
                print('Calculating sliding average using window size of {0} seconds ({1} samples)'.format(swSize, srateCorrectedSW))
            respSlidingMean = self.fftSmooth(thisResp, srateCorrectedSW)
            self.baselineCorrectedRespiration = thisResp - respSlidingMean

        if zScore == 1:
            self.baselineCorrectedRespiration = (self.baselineCorrectedRespiration - np.mean(self.baselineCorrectedRespiration)) / np.std(self.baselineCorrectedRespiration, ddof=1)
    ...
    def findExtrema(self,customDecisionThreshold=0,swSizes=[]):
        ...
        for win in range(len(swSizes)):
            ...
            for shift in SHIFTS:
                ...
                windowInit = int((sw - np.floor(sw / (shift+ 1)))) + 1
    ...
    def findRespiratoryPausesAndOnsets(self,nBins=100):
        ...
        '''
        nBins = max(int(np.floor(self.srate / 100)), 20)
        ...
        UPPER_THRESHOLD = np.round(nBins*.7) - 1
        LOWER_THRESHOLD = np.round(nBins*.3) - 1
        ...
        edges = np.append(CUSTOM_BINS - bin_width/2, CUSTOM_BINS[-1] + bin_width/2)
        # ensure edges cover full range
        edges[0] = min(THIS_WINDOW)
        edges[-1] = max(THIS_WINDOW)
        [AMPLITUDE_VALUES, WINDOW_BINS] = np.histogram(THIS_WINDOW, bins=edges)
        MODE_BIN = np.argmax(AMPLITUDE_VALUES)
        ZERO_CROSS_THRESHOLD =  (WINDOW_BINS[MODE_BIN] + WINDOW_BINS[MODE_BIN + 1]) / 2 # WINDOW_BINS[MODE_BIN]
        ...
        if sum(POSSIBLE_INHALE_INDS) > 0:
            ...
            inhaleOnsets[0,0] = firstZeroCrossBoundary + INHALE_ONSET + 1
        else:
        ...
        for THIS_BREATH in range(len(peaks)-1):
            ...
            edges = np.append(CUSTOM_BINS - bin_width/2, CUSTOM_BINS[-1] + bin_width/2)
            # ensure edges cover full range
            edges[0] = min(INHALE_WINDOW)
            edges[-1] = max(INHALE_WINDOW)
            [AMPLITUDE_VALUES, WINDOW_BINS] = np.histogram(INHALE_WINDOW, bins=edges)
            MODE_BIN = np.argmax(AMPLITUDE_VALUES)
            ...
            if ~isExhalePause:
                ...
                inhaleOnsets[0, THIS_BREATH + 1] = troughs[THIS_BREATH] + INHALE_ONSET + 1
...
#                     print('no exhale pause found')
                    ...
                    exhalePauseOnsets[0, THIS_BREATH] = troughs[THIS_BREATH] + PAUSE_ONSET + 1
                    inhaleOnsets[0, THIS_BREATH + 1] = troughs[THIS_BREATH] + INHALE_ONSET + 1
            ...
            edges = np.append(CUSTOM_BINS - bin_width/2, CUSTOM_BINS[-1] + bin_width/2)
            # ensure edges cover full range
            edges[0] = min(EXHALE_WINDOW)
            edges[-1] = max(EXHALE_WINDOW)

            [AMPLITUDE_VALUES, WINDOW_BINS] = np.histogram(EXHALE_WINDOW, bins=edges)
            MODE_BIN = np.argmax(AMPLITUDE_VALUES)
            MAX_BIN_RATIO = AMPLITUDE_VALUES[MODE_BIN]/np.mean(AMPLITUDE_VALUES)
            ...
            if ~isInhalePause:
                ...
                exhaleOnsets[0, THIS_BREATH] = peaks[THIS_BREATH] + EXHALE_ONSET + 1
...
#                     print('no inhale pause found')
                    ...
                    inhalePauseOnsets[0, THIS_BREATH] = peaks[THIS_BREATH] + PAUSE_ONSET + 1
                    exhaleOnsets[0, THIS_BREATH] = peaks[THIS_BREATH] + EXHALE_ONSET + 1
        ...
        if sum(POSSIBLE_EXHALE_INDS)>0:
            ...
            exhaleOnsets[0,-1] = peaks[-1] + EXHALE_BEST_GUESS[0][0] + 1
        else:
    ...
    def findRespiratoryOffsets(self):
        ...
        avgExhaleLen = np.mean(exhaleOffsets[0, :exhaleOffsets.shape[1]-1] - self.exhaleOnsets[:len(self.exhaleOnsets)-1])
        lowerLim = avgExhaleLen / 4;
        upperLim = avgExhaleLen * 1.75
        if putativeExhaleOffsets[0].size>0:
            if putativeExhaleOffsets[0][0] < lowerLim or putativeExhaleOffsets[0][0] >= upperLim:  #or?
    ...
    def estimateBreathVolumes(self):
        ...
        for bi in range(len(self.inhaleOnsets)):
            if np.isnan(self.inhaleOffsets[0,bi]):
                ...
                thisInhale = np.arange(self.inhaleOnsets[bi],int(self.inhaleOffsets[0,bi])+1)
        ...
        for bi in range(len(self.exhaleOnsets)):
            if np.isnan(self.exhaleOffsets[0,bi]):
                ...
                thisExhale = np.arange(self.exhaleOnsets[bi],int(self.exhaleOffsets[0,bi])+1)
    ...
    def estimateAllFeatures(self, zScore=0, baselineCorrectionMethod='sliding', simplify=1, verbose=0 ):
        ...
        if self.dataType == 'humanAirflow' or self.dataType == 'rodentAirflow':
            self.smoothResp()
            self.correctRespirationToBaseline(method=baselineCorrectionMethod,zScore=zScore, verbose=verbose)
